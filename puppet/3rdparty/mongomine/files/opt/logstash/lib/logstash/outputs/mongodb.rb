  ...
  def receive(event)
    ...
    begin
      ...
      if @isodate
        ...
        document = copy_event.to_hash
        document["t"] = document["@timestamp"]
      else
        document = copy_event.to_hash.merge("@timestamp" => copy_event["@timestamp"].to_json)
      end
      if @generateId
        document['_id'] = BSON::ObjectId.new(nil, copy_event["@timestamp"])
      end
